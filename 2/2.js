...
var checksum = function(){
    ...
    for(var i =0; i<rows.length; i++){
        numbersLine=(rows[i].split("\t"));
        allNumbers=numbersLine.map(x => parseInt(x));
        allToSum[i]=highest(allNumbers)-lowest(allNumbers);
    }

    for(var j=0; j<allToSum.length; j++){
        answer += allToSum[j];
    }
    return answer;
}

var highest = function(vector){
    var temp = vector[0];
...
var checksum2 = function(){
    ...
    for(var i =0; i<rows.length; i++){
        numbersLine=(rows[i].split("\t"));
        allNumbers=numbersLine.map(x => parseInt(x));
        for(var a=0; a<allNumbers.length; a++){
            for(var b=0; b<allNumbers.length; b++){
                if(canBeDivided(allNumbers[a], allNumbers[b])&& a!=b){
                    allToSum[i]=(allNumbers[a]/allNumbers[b]);
                }
            }
        }
    }
    for(var j=0; j<allToSum.length; j++){
        answer2 += allToSum[j];
        console.log(allToSum[j]);
    }
    return answer2;
}

var canBeDivided = function(a, b){
    if(a%b===0){
        return true;
    }
    return false;
